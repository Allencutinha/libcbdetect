...
void filter_corners(const cv::Mat &img, const cv::Mat &img_angle, const cv::Mat &img_weight,
    ...
    cv::parallel_for_(cv::Range(0, corners.p.size()), [&](const cv::Range &range) -> void {
        for (int i = range.start; i < range.end; ++i) {
            ...
            int r = corners.r[i];
            if (center_u - r < 0 || center_u + r >= width - 1 || center_v - r < 0 ||
                center_v + r >= height - 1) {
                continue;
            }
            ...
            for (int j = fisrt_cross_index, count = 1; j < n_cicle + fisrt_cross_index;
                 ++j, ++count) {
            ...
            for (int j2 = top_left_v; j2 <= bottom_right_v; ++j2) {
                for (int i2 = top_left_u; i2 <= bottom_right_u; ++i2) {
                    int bin = static_cast<int>(
                                  std::floor(img_angle.at<double>(j2, i2) / (M_PI / n_bin))) %
                              n_bin;
                    angle_hist[bin] +=
                        img_weight_sub.at<double>(j2 - center_v + r, i2 - center_u + r);
                }
            }
