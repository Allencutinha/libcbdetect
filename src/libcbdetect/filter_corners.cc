      ...
      cv::Range(0, corners.p.size()), [&](const cv::Range &range) -> void {
        for (int i = range.start; i < range.end; ++i) {
          ...
          for (int j = 0; j < n_cicle; ++j) {
            int circle_u =
                static_cast<int>(std::round(center_u + 0.75 * r * cos_v[j]));
            int circle_v =
                static_cast<int>(std::round(center_v + 0.75 * r * sin_v[j]));
          ...
          img_weight.rowRange(top_left_v, bottom_right_v + 1)
              ...
              .copyTo(
                  img_weight_sub(cv::Range(top_left_v - center_v + r,
          ...
          for (int j2 = top_left_v; j2 <= bottom_right_v; ++j2) {
            for (int i2 = top_left_u; i2 <= bottom_right_u; ++i2) {
              int bin = static_cast<int>(std::floor(
                            img_angle.at<double>(j2, i2) / (M_PI / n_bin))) %
                        n_bin;
              angle_hist[bin] += img_weight_sub.at<double>(j2 - center_v + r,
                                                           i2 - center_u + r);
            }
          }
