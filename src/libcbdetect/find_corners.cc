...
void find_corners_reiszed(const cv::Mat &img, Corner &corners,
  ...
  cv::resize(img, img_resized, cv::Size(img.cols * scale, img.rows * scale), 0,
             0, cv::INTER_LINEAR);
  ...
  // std::for_each(corners_resized.r.begin(), corners_resized.r.end(),
  // [&scale](auto &r) { r = (double) r / scale; });
