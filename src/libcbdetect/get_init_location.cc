...
void rotate_image(const cv::Mat &img_in, double angle, cv::Mat &img_out,
  ...
  //      sin(angle) + center_u; double v = (i - new_center_u) * std::sin(angle)
...
void get_init_location(const cv::Mat &img, const cv::Mat &img_du,
  ...
  case TemplateMatchSlow: {
    ...
    if (detect_method == TemplateMatchFast) {
      ...
      tprops = {0,        M_PI_2,    M_PI_4,        -M_PI_4,      0,
    ...
    for (const auto &r : params.radius) {
      ...
      cv::Mat img_corners_a1, img_corners_a2, img_corners_b1, img_corners_b2,
          img_corners_mu, img_corners_a, img_corners_b, img_corners_s1,
          img_corners_s2;
      ...
      for (int i = 0; i < tprops.size(); i += 2) {
        ...
        // compute mean
        img_corners_mu = (img_corners_a1 + img_corners_a2 + img_corners_b1 +
                          img_corners_b2) /
                         4;
        ...
        // combine both
        img_corners =
            cv::max(img_corners, cv::max(img_corners_s1, img_corners_s2));
      }
  ...
  case LocalizedRadonTransform: {
    ...
    for (const auto &r : params.radius) {
      non_maximum_suppression(response_img, r, params.init_loc_thr / 10.0, r,
                              corners);
    }
    break;
  }
        ...
        for (int i = range.start; i < range.end; ++i) {
          ...
          for (int j2 = 0; j2 < 2 * r + 1; ++j2) {
            for (int i2 = 0; i2 < 2 * r + 1; ++i2) {
              ...
              b.at<double>(0, 0) +=
                  o_du * o_du * (i2 - r + u) + o_du * o_dv * (j2 - r + v);
              b.at<double>(1, 0) +=
                  o_du * o_dv * (i2 - r + u) + o_dv * o_dv * (j2 - r + v);
            }
          }
