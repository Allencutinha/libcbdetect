...
void hessian_response(const cv::Mat &img_in, cv::Mat &img_out) {
    ...
    cv::parallel_for_(cv::Range(1, rows - 1),
...
void localized_radon_transform(const cv::Mat &img_in, cv::Mat &img_out) {
    ...
    img_out.forEach<double>(
        [](double &pixel, const int *position) -> void { pixel = pixel * pixel; });
}
...
void get_init_location(const cv::Mat &img, const cv::Mat &img_du, const cv::Mat &img_dv,
    ...
    DetectMethod detect_method =
        params.corner_type == MonkeySaddlePoint ? HessianResponse : params.detect_method;
    ...
    case TemplateMatchSlow: {
        ...
        for (const auto &r : params.radius) {
            ...
            for (int i = 0; i < tprops.size(); i += 2) {
                ...
                cv::filter2D(img, img_corners_a1, -1, template_kernel[0], cv::Point(-1, -1), 0,
                             cv::BORDER_REPLICATE);
                cv::filter2D(img, img_corners_a2, -1, template_kernel[1], cv::Point(-1, -1), 0,
                             cv::BORDER_REPLICATE);
                cv::filter2D(img, img_corners_b1, -1, template_kernel[2], cv::Point(-1, -1), 0,
                             cv::BORDER_REPLICATE);
                cv::filter2D(img, img_corners_b2, -1, template_kernel[3], cv::Point(-1, -1), 0,
                             cv::BORDER_REPLICATE);

                // compute mean
                img_corners_mu =
                    (img_corners_a1 + img_corners_a2 + img_corners_b1 + img_corners_b2) / 4;
