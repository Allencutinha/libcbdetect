...
void hessian_response(const cv::Mat &img_in, cv::Mat &img_out) {
    ...
    cv::parallel_for_(
        cv::Range(1, rows - 1),
        [&img_in, &img_out, &stride, &cols](const cv::Range &range) -> void {
            // setup input and output pointer to be centered at 1,0 and 1,1
            // resp.
...
void rotate_image(const cv::Mat &img_in, double angle, cv::Mat &img_out,
    ...
    if (out_size.width <= 0 || out_size.height <= 0) {
        cv::Point2i tl(std::round(-in_center_u * std::cos(angle) -
                                  in_center_v * std::sin(angle)),
                       std::round(in_center_u * std::sin(angle) -
                                  in_center_v * std::cos(angle)));
        cv::Point2i tr(std::round(in_center_u * std::cos(angle) -
                                  in_center_v * std::sin(angle)),
                       std::round(-in_center_u * std::sin(angle) -
                                  in_center_v * std::cos(angle)));
        cv::Point2i bl(std::round(-in_center_u * std::cos(angle) +
                                  in_center_v * std::sin(angle)),
                       std::round(in_center_u * std::sin(angle) +
                                  in_center_v * std::cos(angle)));
        cv::Point2i br(std::round(in_center_u * std::cos(angle) +
                                  in_center_v * std::sin(angle)),
                       std::round(-in_center_u * std::sin(angle) +
                                  in_center_v * std::cos(angle)));
        if (std::min(tl.x, br.x) > std::min(tr.x, bl.x)) {
            out_size =
                cv::Size(std::abs(tr.x - bl.x) + 1, std::abs(tl.y - br.y) + 1);
        } else {
            out_size =
                cv::Size(std::abs(tl.x - br.x) + 1, std::abs(tr.y - bl.y) + 1);
        }
    }
    ...
    //      double u = (i - new_center_u) * std::cos(angle) - (j - new_center_v)
    //      * sin(angle) + center_u; double v = (i - new_center_u) *
    //      std::sin(angle)
    //      + (j - new_center_v) * cos(angle) + center_v; if (u < 0 || u >=
    //      img_in.cols - 1 || v < 0 || v >= img_in.rows - 1) {
    ...
    //      double a10 = dv - du * dv;
    //      double a11 = du * dv;
    //      img_out.at<double>(j, i) = img_in.at<double>(v, u) * a00 +
    //      img_in.at<double>(v, u + 1) * a01 +
    //          img_in.at<double>(v + 1, u) * a10 + img_in.at<double>(v + 1, u +
    //          1) * a11;
    //    }
    //  }

    // rotate image
    double shift_u = out_center_u - in_center_u * std::cos(angle) -
                     in_center_v * std::sin(angle);
    double shift_v = out_center_v + in_center_u * std::sin(angle) -
                     in_center_v * std::cos(angle);
    cv::Mat rot = (cv::Mat_<double>(2, 3) << std::cos(angle), std::sin(angle),
                   shift_u, -std::sin(angle), std::cos(angle), shift_v);
...
void get_init_location(const cv::Mat &img, const cv::Mat &img_du,
                       ...
                       const Params &params) {
    DetectMethod detect_method = params.corner_type == MonkeySaddlePoint
                                     ? HessianResponse
                                     : params.detect_method;
    ...
    case TemplateMatchSlow: {
        ...
        if (detect_method == TemplateMatchFast) {
            ...
            tprops = {
                0,        M_PI_2,    M_PI_4,        -M_PI_4,      0,
                M_PI_4,   0,         -M_PI_4,       M_PI_4,       M_PI_2,
                -M_PI_4,  M_PI_2,    -3 * M_PI / 8, 3 * M_PI / 8, -M_PI / 8,
                M_PI / 8, -M_PI / 8, -3 * M_PI / 8, M_PI / 8,     3 * M_PI / 8};
        }
        ...
        for (const auto &r : params.radius) {
            ...
            cv::Mat img_corners_a1, img_corners_a2, img_corners_b1,
                img_corners_b2, img_corners_mu, img_corners_a, img_corners_b,
                img_corners_s1, img_corners_s2;

            for (int i = 0; i < tprops.size(); i += 2) {
                std::vector<cv::Mat> template_kernel(4); // a1, a2, b1, b2
                create_correlation_patch(template_kernel, tprops[i],
                                         tprops[i + 1], r);

                // filter image with current template
                cv::filter2D(img, img_corners_a1, -1, template_kernel[0],
                             cv::Point(-1, -1), 0, cv::BORDER_REPLICATE);
                cv::filter2D(img, img_corners_a2, -1, template_kernel[1],
                             cv::Point(-1, -1), 0, cv::BORDER_REPLICATE);
                cv::filter2D(img, img_corners_b1, -1, template_kernel[2],
                             cv::Point(-1, -1), 0, cv::BORDER_REPLICATE);
                cv::filter2D(img, img_corners_b2, -1, template_kernel[3],
                             cv::Point(-1, -1), 0, cv::BORDER_REPLICATE);
                ...
                img_corners_a =
                    cv::min(img_corners_a1, img_corners_a2) - img_corners_mu;
                img_corners_b =
                    img_corners_mu - cv::max(img_corners_b1, img_corners_b2);
                img_corners_s1 = cv::min(img_corners_a, img_corners_b);
                // case 2: b=white, a=black
                img_corners_a =
                    img_corners_mu - cv::max(img_corners_a1, img_corners_a2);
                img_corners_b =
                    cv::min(img_corners_b1, img_corners_b2) - img_corners_mu;
                img_corners_s2 = cv::min(img_corners_a, img_corners_b);

                // combine both
                img_corners = cv::max(img_corners,
                                      cv::max(img_corners_s1, img_corners_s2));
            }
            non_maximum_suppression(img_corners, 1, params.init_loc_thr, r,
                                    corners);
        }
        break;
    }
    ...
    case LocalizedRadonTransform: {
        ...
        for (const auto &r : params.radius) {
            non_maximum_suppression(response_img, r, params.init_loc_thr / 10.0,
                                    r, corners);
        }
        break;
    }
            ...
            for (int i = range.start; i < range.end; ++i) {
                ...
                for (int j2 = 0; j2 < 2 * r + 1; ++j2) {
                    for (int i2 = 0; i2 < 2 * r + 1; ++i2) {
                        ...
                        b.at<double>(0, 0) += o_du * o_du * (i2 - r + u) +
                                              o_du * o_dv * (j2 - r + v);
                        b.at<double>(1, 0) += o_du * o_dv * (i2 - r + u) +
                                              o_dv * o_dv * (j2 - r + v);
                    }
                }
