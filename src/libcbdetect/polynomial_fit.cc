  ...
  cv::parallel_for_(
      cv::Range(0, corners.p.size()), [&](const cv::Range &range) -> void {
        for (int i = range.start; i < range.end; ++i) {
          ...
          // fit f(x, y) = k0 * x^2 + k1 * y^2 + k2 * x * y + k3 * x + k4 * y +
          // k5 coef: [k0; k1; k2; k3; k4; k5]
          for (int num_it = 0; num_it < max_iteration; ++num_it) {
            cv::Mat k, b;
            if (u_cur - r < 0 || u_cur + r >= width - 1 || v_cur - r < 0 ||
                v_cur + r >= height - 1) {
            ...
            double dist = std::sqrt((u_cur - u_init) * (u_cur - u_init) +
  ...
  cv::parallel_for_(
      cv::Range(0, corners.p.size()), [&](const cv::Range &range) -> void {
        for (int i = range.start; i < range.end; ++i) {
          ...
          // fit f(x, y) = k0 * x^3 + k1 * x^2*y + k2* x*y^2 + k3 * y^3 + k4 *
          // x^2 + k5 * x*y + k6 * y^2 + k7 * x + k8 * y + k9 coef: [k0; k1; k2;
          ...
          for (int num_it = 0; num_it < max_iteration; ++num_it) {
            ...
            // the monkey saddle point is a degenerate critical point where all
            // of its second derivatives are zero
            cv::Mat tmp_a = (cv::Mat_<double>(3, 2) << 3.0 * k.at<double>(0, 0),
                             k.at<double>(1, 0), 2.0 * k.at<double>(1, 0),
                             2.0 * k.at<double>(2, 0), k.at<double>(2, 0),
                             3.0 * k.at<double>(3, 0));
            cv::Mat tmp_b = (cv::Mat_<double>(3, 1) << -k.at<double>(4, 0),
...
void polynomial_fit(const cv::Mat &img, Corner &corners, const Params &params) {
  if (params.corner_type == SaddlePoint) {
    ...
    polynomial_fit_monkey_saddle(img, params.polynomial_fit_half_kernel_size,
                                 corners);
  }
}
