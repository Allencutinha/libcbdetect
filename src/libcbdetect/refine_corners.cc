...
std::vector<std::vector<double>> edge_orientations(cv::Mat &img_angle,
  ...
  for (int i = 0; i < img_angle.cols; ++i) {
    for (int j = 0; j < img_angle.rows; ++j) {
      int bin =
          static_cast<int>(std::floor(img_angle.at<double>(j, i) / (M_PI / n)));
        ...
        for (int i = range.start; i < range.end; ++i) {
          ...
          for (int j2 = vi - r; j2 <= vi + r; ++j2) {
            for (int i2 = ui - r; i2 <= ui + r; ++i2) {
              ...
              if (is_monkey_saddle &&
                  std::abs(o_du_norm * v[2][0] + o_dv_norm * v[2][1]) < 0.25) {
          ...
          for (int num_it = 0; num_it < max_iteration; ++num_it) {
            ...
            double dist = std::sqrt((u_cur - u_last) * (u_cur - u_last) +
