    ...
    for (int i = 0; i < img_angle.cols; ++i) {
        for (int j = 0; j < img_angle.rows; ++j) {
            int bin = static_cast<int>(
                std::floor(img_angle.at<double>(j, i) / (M_PI / n)));
...
std::vector<std::vector<double>> edge_3_orientations(cv::Mat &img_angle,
    ...
    double delta_angle_1 =
        std::min(angle_2 - angle_1, angle_1 + M_PI - angle_2);
    double delta_angle_2 =
        std::min(angle_3 - angle_2, angle_2 + M_PI - angle_3);
    double delta_angle_3 =
        std::min(angle_3 - angle_1, angle_1 + M_PI - angle_3);
...
void refine_corners(const cv::Mat &img_du, const cv::Mat &img_dv,
                    const cv::Mat &img_angle, const cv::Mat &img_weight,
    ...
    cv::parallel_for_(
        cv::Range(0, corners.p.size()), [&](const cv::Range &range) -> void {
            for (int i = range.start; i < range.end; ++i) {
                ...
                auto v =
                    is_monkey_saddle
                        ? edge_3_orientations(img_angle_sub, img_weight_sub)
                ...
                for (int j2 = vi - r; j2 <= vi + r; ++j2) {
                    for (int i2 = ui - r; i2 <= ui + r; ++i2) {
                        ...
                        if (std::abs(o_du_norm * v[0][0] +
                                     o_dv_norm * v[0][1]) < 0.25) {
                        ...
                        if (std::abs(o_du_norm * v[1][0] +
                                     o_dv_norm * v[1][1]) < 0.25) {
                        ...
                        if (is_monkey_saddle &&
                            std::abs(o_du_norm * v[2][0] +
                                     o_dv_norm * v[2][1]) < 0.25) {
                ...
                double u_cur = u_init, v_cur = v_init, u_last = u_cur,
                       v_last = v_cur;
                for (int num_it = 0; num_it < max_iteration; ++num_it) {
                    ...
                    if (u_cur - r < 0 || u_cur + r >= width || v_cur - r < 0 ||
                        v_cur + r >= height) {
                        break;
                    }
                    ...
                    for (int j2 = 0; j2 < 2 * r + 1; ++j2) {
                        for (int i2 = 0; i2 < 2 * r + 1; ++i2) {
                            ...
                            // compute rel. position of pixel and distance to
                            // vectors
                            double w_u =
                                i2 - r -
                                ((i2 - r) * v[0][0] + (j2 - r) * v[0][1]) *
                                    v[0][0];
                            double v_u =
                                j2 - r -
                                ((i2 - r) * v[0][0] + (j2 - r) * v[0][1]) *
                            ...
                            // directions
                            if ((d1 < 3 &&
                                 std::abs(o_du_norm * v[0][0] +
                                          o_dv_norm * v[0][1]) < 0.25) ||
                                (d2 < 3 &&
                                 std::abs(o_du_norm * v[1][0] +
                                          o_dv_norm * v[1][1]) < 0.25)) {
                                ...
                                b.at<double>(0, 0) +=
                                    o_du * o_du * (i2 - r + u_cur) +
                                    o_du * o_dv * (j2 - r + v_cur);
                                b.at<double>(1, 0) +=
                                    o_du * o_dv * (i2 - r + u_cur) +
                                    o_dv * o_dv * (j2 - r + v_cur);
                            }

                            if (is_monkey_saddle) {
                                w_u =
                                    i2 - r -
                                    ((i2 - r) * v[2][0] + (j2 - r) * v[2][1]) *
                                        v[2][0];
                                v_u =
                                    j2 - r -
                                    ((i2 - r) * v[2][0] + (j2 - r) * v[2][1]) *
                    ...
                    double dist =
                        std::sqrt((u_cur - u_last) * (u_cur - u_last) +
                                  (v_cur - v_last) * (v_cur - v_last));
                    if (dist >= 3) {
                        u_cur = u_last;
                        v_cur = v_last;
                        break;
                ...
                // add to corners
                if (std::sqrt((u_cur - u_init) * (u_cur - u_init) +
